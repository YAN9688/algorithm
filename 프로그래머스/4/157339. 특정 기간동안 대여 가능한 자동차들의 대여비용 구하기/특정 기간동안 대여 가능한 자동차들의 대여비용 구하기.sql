SELECT DISTINCT A.CAR_ID, A.CAR_TYPE, ROUND(A.DAILY_FEE*(100-C.DISCOUNT_RATE)*30/100,0) AS FEE
FROM CAR_RENTAL_COMPANY_CAR AS A
LEFT JOIN CAR_RENTAL_COMPANY_RENTAL_HISTORY AS B
ON A.CAR_ID = B.CAR_ID
LEFT JOIN CAR_RENTAL_COMPANY_DISCOUNT_PLAN AS C
ON A.CAR_TYPE = C.CAR_TYPE
WHERE A.CAR_TYPE IN ('세단', 'SUV')
AND (C.DURATION_TYPE = '30일 이상' AND (A.DAILY_FEE*(100-C.DISCOUNT_RATE)*30/100) BETWEEN 500000 AND 2000000)
AND (A.CAR_ID NOT IN(SELECT CAR_ID
                    FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
                    WHERE '2022-11' BETWEEN DATE_FORMAT(START_DATE,'%Y-%m') AND DATE_FORMAT(END_DATE,'%Y-%m')))
ORDER BY FEE DESC, A.CAR_TYPE ASC, A.CAR_ID DESC
